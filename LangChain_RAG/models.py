...
ALI_TEXT_EMBEDDING_V4 = "text-embedding-v4"
...
def get_lc_model_client(
    ...
    if debug:
        print(
            f"{function_name}-平台：{base_url},模型：{model},温度：{temperature},key：{api_key}"
        )
    return ChatOpenAI(
        api_key=api_key,
        base_url=base_url,
...
def get_ali_model_client(
    model=ALI_TONGYI_DEEPSEEK_V3, temperature=0.7, verbose=False, debug=False
):
...
def get_tencent_model_client(
    model=TENCENT_HUNYUAN_TURBO_MODEL, temperature=0.7, verbose=False, debug=False
):
    """通过LangChain使用腾讯大模型"""
    return get_lc_model_client(
        api_key=os.getenv(TENCENT_HUNYUAN_API_KEY_OS_VAR_NAME),
...
def get_ds_model_client(
    model=DEEPSEEK_CHAT_MODEL, temperature=0.7, verbose=False, debug=False
):
...
def get_baichuan_embeddings():
    ...
    return BaichuanTextEmbeddings(api_key=os.getenv(BAICHUAN_API_KEY_OS_VAR_NAME))
